...
local function is_absolute(path)
  ...
  if IS_WINDOWS and path:match("^%a:/") then return true end
  return false
end
...
local function windows_symlink_info(path)
  ...
  for line in output:gmatch("[^\r\n]+") do
    ...
    if link_name then
      ...
      if link_name == name then return true, to_forward_slash(target) end
    end
  end
...
function module.is_symlink(path)
  if IS_WINDOWS then return (windows_symlink_info(path)) end
...
function module.symlink_target(path)
  ...
  local attr = lfs.symlinkattributes(path)
  if attr and attr.mode == "link" then return attr.target end
  return nil
end
...
function module.mkdir_p(path)
  if module.exists(path) then return true end
  ...
  if not module.exists(path) then return lfs.mkdir(path) end
...
function module.resolve_symlink(path)
  ...
  if is_absolute(target) then return target end
...
function module.normalize(path)
  ...
  if abs then return drive_prefix .. "/" .. normalized end
